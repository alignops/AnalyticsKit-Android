 ...
 * @author Trevor
 */
public class LevelStartLoggerTest extends LoggerBaseTest
{
	LevelStartLogger logger;
	AnalyticsEvent   levelStartEvent;
	...
	public void setup()
	{
		...
		logger = new LevelStartLogger(answers);
		levelStartEvent = new AnalyticsEvent(PredefinedEvents.LEVEL_START).putAttribute(Attributes.LevelStart.LEVEL_NAME, "First Level")
	...
	{
		LevelStartEvent result = logger.buildAnswersLevelStartEvent(levelStartEvent);
		...
		assertThat(predefinedAttributes).containsKey(Attributes.LevelStart.LEVEL_NAME);
	...
	{
		logger.logSpecificEvent(levelStartEvent);

		assertThat(answers.logLevelStartCalled).isTrue();
		assertThat(answers.levelStartEvent).isNotNull();
	}
}
